  ...
  _init(options) {
    ...
    const onmousemove = function (e) {
      ...
      if (settings.snapDistance > 0) {
        ...
        me._initDiffSiblings.map(initDiffSibling => {
          if (
            snap(initDiffSibling, 'topR', false, settings.snapGap) ||
            snap(initDiffSibling, 'bottomR', false, -settings.snapGap)
          ) {
            snap(initDiffSibling, 'left', false, 0, true) || snap(initDiffSibling, 'right', false, 0, true);
          } else if (
            snap(initDiffSibling, 'leftR', false, settings.snapGap) ||
            snap(initDiffSibling, 'rightR', false, -settings.snapGap)
          ) {
    ...
    const onmousedown = function (e) {
      ...
      if (settings.snapDistance > 0) {
        ...
        me._initDiffSiblings = siblings(node, '.azui')
          .filter(o => {
